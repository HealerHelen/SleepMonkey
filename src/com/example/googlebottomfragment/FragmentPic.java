...
/*			R.drawable.img09, R.drawable.img10, R.drawable.img11,
    ...
    public View onCreateView(LayoutInflater inflater,
		...
		imageSwitcher = (ImageSwitcher)view.findViewById(R.id.imageSwitcher1); // 获取图像切换器
		// 设置动画效果
		...
		imageSwitcher.setFactory(new ViewFactory() {
			...
			public View makeView() {
				ImageView imageView = new ImageView(getActivity().getApplicationContext()); // 实例化一个ImageView类的对象
		...
		imageSwitcher.setImageResource(imageId[0]);// 设置默认显示的图像
		GridView gridview = (GridView) view.findViewById(R.id.gridView1); // 获取GridView组件
		...
		BaseAdapter adapter=new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				if(convertView==null){
					imageview=new ImageView(getActivity().getApplicationContext());	//实例化ImageView的对象
					/*************设置图像的宽度和高度******************/
